...
void calcular_jugadas(tcarta descarte, tbaraja b, tjugadas *j)
{
    ...
    {
        if(descarte.color==b.cartas[i].color || descarte.figura==b.cartas[i].figura || b.cartas[i].figura==13 || b.cartas[i].figura==14)
        {
...
int hay_jugada(tcarta mazodescartes, tbaraja mazojugador) //mazo descarte esta en tpartida//
{
    ...
    {
        if(mazodescartes.color==mazojugador.cartas[i].color || mazodescartes.figura==mazojugador.cartas[i].figura || mazojugador.cartas[i].figura==13 || mazojugador.cartas[i].figura==14)
        {
            jugadas=1;
        }
        i++;
...
int seleccionar_jugada(tjugadas numjugadas, tpartida p)
{
    ...
    if(p.Js.turno==0)
    {
        do
        {
            ...
            scanf("%d%*c", &opc);
        }while(opc<0 || opc>numjugadas.num_jugadas-1);
    }
    else
    {
